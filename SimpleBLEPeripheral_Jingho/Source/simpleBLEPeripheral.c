...
#define SBP_PERIODIC_TSK_PERIOD                   800
...
static void ValueNotification(){    
    ...
    while(HEAD_BUFFER->count!=HEAD_BUFFER->cur && (SeqPkt--)!=0){
      if((BufferQueue[HEAD_BUFFER->cur][0]!=BufferQueue[HEAD_BUFFER->cur+1][0])||(HEAD_BUFFER->cur+1==HEAD_BUFFER->count))
        BufferQueue[HEAD_BUFFER->cur][9]=0xFF;
      else
        BufferQueue[HEAD_BUFFER->cur][9]=0x00;
