...
} PeriodicEvent_Create;

typedef struct Buffer Buffer;
struct Buffer{
    uint8 size;
    uint8 count;    //store address
    uint8 cur;      //Current address
    
    uint8 data[30][20];   //a pointer to array[20] of char
};

/*********************************************************************
 * GLOBAL VARIABLES
 */
  extern PeriodicEvent_Create PERIODIC_EVENT[];
  extern Buffer *HEAD_BUFFER;
